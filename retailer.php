...
$dist_list = mysqli_query($link, "SELECT id, distributor_name FROM distributors WHERE status='active' ORDER BY distributor_name ASC");

// Add / Edit
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['retailer_name'])){
...
if(isset($_GET['delete'])){
    $del = (int)$_GET['delete'];
    mysqli_query($link, "DELETE FROM retailers WHERE id=$del");
    header("location:retailer.php"); exit();
}
...
if(isset($_GET['edit'])){
    $eid = (int)$_GET['edit'];
    $edit_row = mysqli_fetch_assoc(mysqli_query($link, "SELECT * FROM retailers WHERE id=$eid"));
}

$records = mysqli_query($link,
    "SELECT r.*, d.distributor_name 
     FROM retailers r 
     LEFT JOIN distributors d ON r.distributor_id = d.id 
     ORDER BY r.id DESC");
$total  = mysqli_num_rows($records);
$active = mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(*) as c FROM retailers WHERE status='active'"))['c'];
?>
...
<div class="container-fluid">
    ...
    <div class="row mb-3">
        <div class="col-md-3">
            <div class="card text-white bg-info text-center py-3">
                <h3><?= $total ?></h3><p class="mb-0">Total Retailers</p>
            </div>
        </div>
        <div class="col-md-3">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-header">
                    ...
                    <?= $edit_row ? 'Edit Retailer' : 'Add Retailer' ?></strong>
                    <?php if($edit_row): ?>
                        <a href="retailer.php" class="btn btn-sm btn-secondary float-right">
                ...
                <div class="card-body">
                    ...
                    <form method="POST">
                        ...
                        <div class="form-group">
                            <label><b>Retailer Name *</b></label>
                            <input type="text" name="retailer_name" class="form-control" required
                                   placeholder="Retailer ka naam"
                                   value="<?= $edit_row ? htmlspecialchars($edit_row['retailer_name']) : '' ?>">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="distributor_id" class="form-control">
                                ...
                                <?php
                                mysqli_data_seek($dist_list, 0);
                                while($d = mysqli_fetch_assoc($dist_list)):
                                    $sel = ($edit_row && $edit_row['distributor_id']==$d['id']) ? 'selected' : '';
                                ?>
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header">
                    <strong><i class="fa fa-list"></i> Retailers List</strong>
                    <span class="badge badge-info float-right mt-1"><?= $total ?> Total</span>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-bordered table-striped table-hover table-sm">
                            ...
                            <tr <?= ($edit_row && $edit_row['id']==$row['id']) ? 'class="table-warning"' : '' ?>>
                                ...
                                <td>
                                    <?php if($row['distributor_name']): ?>
                                        <span class="badge badge-info"><?= htmlspecialchars($row['distributor_name']) ?></span>
                                    <?php else: ?>
                                ...
                                <td>
                                    <a href="retailer.php?edit=<?= $row['id'] ?>"
                                       class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>
                                    <a href="retailer.php?delete=<?= $row['id'] ?>"
                        ...
                        <?php endwhile; ?>
                        <?php if($total == 0): ?>
                            <tr><td colspan="8" class="text-center text-muted">Koi retailer nahi hai abhi tak.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                    </div>
